...
const MembershipManagement = () => {
  ...
  const fetchEntityTypes = async () => {
    try {
      const response = await fetch("https://api.cuintranet.in/intranetapp/entity-types/")
  ...
  const handleEditFormSubmit = async () => {
    try {
      const response = await apiClient.put(`/membership/${currentItemId}/update/`, editFormData)
  ...
  const handleConfirmDelete = async () => {
    try {
      const response = await apiClient.delete(`/membership/${deleteId}/delete/`)

      if (response.status === 200) {
        // Remove the deleted item from the table data
        const updatedData = tableData.filter((item) => item.member_id !== deleteId)
        setTableData(updatedData)
  ...
  const handleClusterFormSubmit = async () => {
    try {
      ...
      if (isCreatingCluster) {
        ...
        response = await apiClient.put(`/clusters/${currentClusterId}/detail/`, formData)
        
        if (response.status === 200) {
          // Update the clusters data
          const updatedClusters = clusters.map((cluster) =>
            cluster.cluster_id === currentClusterId ? { ...cluster, ...formData } : cluster
          )
  ...
  const handleConfirmClusterDelete = async () => {
    try {
      const response = await apiClient.delete(`/clusters/${deleteClusterId}/detail/`)

      if (response.status === 204 || response.status === 200) {
        // Remove the deleted cluster from the clusters data
        const updatedClusters = clusters.filter((cluster) => cluster.cluster_id !== deleteClusterId)
        setClusters(updatedClusters)
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.tabs}>
        ...
        <button
          className={`${styles.tabButton} ${activeTab === "clusters" ? styles.activeTab : ""}`}
        ...
        <>
          <div className={styles.filters}>
            <div className={styles.filterGroup}>
              ...
              <select id="entityType" value={selectedEntityId} onChange={handleEntityTypeChange} className={styles.select}>
          ...
          <div className={styles.tableContainer}>
            <div className={styles.tableHeader}>
              ...
              {tableData.length > 0 && <span className={styles.totalCount}>Total: {totalCount}</span>}
            </div>
            ...
            {loading ? (
                      ...
                      <tr key={item.member_id || item.id}>
                        ...
                        <td>
                          <div className={styles.actionButtons}>
                            ...
                            <button className={styles.deleteButton} onClick={() => handleDeleteClick(item.member_id)}>
                              Delete
                            </button>
                          </div>
                        </td>
              ...
              <div className={styles.noData}>Select both entity type and category to view data</div>
            )}
          </div>
            ...
            <div className={styles.drawerOverlay}>
              <div className={styles.drawer}>
                ...
                <div className={styles.drawerFooter}>
                  ...
                  <button className={styles.saveButton} onClick={handleEditFormSubmit}>
                    Save Changes
                  </button>
                </div>
              </div>
      ...
      {activeTab === "clusters" && (
        <>
          ...
          <div className={styles.actionBar}>
            <button className={styles.createButton} onClick={handleCreateClusterClick}>
              Create New Cluster
            </button>
          </div>
          ...
          <div className={styles.tableContainer}>
            <div className={styles.tableHeader}>
              <h2>Clusters List</h2>
              {clusters.length > 0 && <span className={styles.totalCount}>Total: {clusters.length}</span>}
            </div>
                  ...
                  <tbody>
                    {clusters.map((cluster) => (
                      <tr key={cluster.cluster_id}>
                        ...
                        <td>
                          <span className={cluster.status === "active" ? styles.statusActive : styles.statusInactive}>
                            {cluster.status}
                          </span>
                        </td>
                        <td>
                          <div className={styles.actionButtons}>
                            <button className={styles.editButton} onClick={() => handleClusterEditClick(cluster)}>
                              Edit
                            </button>
                            <button className={styles.deleteButton} onClick={() => handleClusterDeleteClick(cluster.cluster_id)}>
                              Delete
                            </button>
                          </div>
                        </td>
          ...
          {showClusterDrawer && (
            <div className={styles.drawerOverlay}>
              <div className={styles.drawer}>
                <div className={styles.drawerHeader}>
                  <h3>{isCreatingCluster ? "Create Cluster" : "Edit Cluster"}</h3>
                  <button className={styles.closeButton} onClick={handleCloseClusterDrawer}>
                    ×
                  </button>
                </div>
                <div className={styles.drawerContent}>
                ...
                <div className={styles.drawerFooter}>
                  <button className={styles.cancelButton} onClick={handleCloseClusterDrawer}>
                    Cancel
                  </button>
                  <button className={styles.saveButton} onClick={handleClusterFormSubmit}>
          ...
          {showClusterDeleteModal && (
            <div className={styles.modalOverlay}>
              <div className={styles.modal}>
                ...
                <div className={styles.modalButtons}>
                  <button className={styles.cancelButton} onClick={handleCancelClusterDelete}>
                    Cancel
                  </button>
                  <button className={styles.deleteButton} onClick={handleConfirmClusterDelete}>
                    Delete
                  </button>
                </div>
              </div>
