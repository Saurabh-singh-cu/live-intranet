...
const JoinNow = () => {
  ...
  const handleFinalSubmit = async () => {
    setLoading(true);
    try {
      // If payment is not required, send empty transaction_id
      const response = await apiClient.post("final_submit/", {
        membership_id: membershipId,
        transaction_id: paymentRequired ? transactionId : "",
      });
  ...
  const handleVerifyOTP = async () => {
    ...
    try {
      ...
      // Check if payment is required from the response
      if (response?.data?.payment_required === true) {
        setPaymentRequired(true);
        setShowPayment(true);
      } else {
        // If payment is not required, don't show payment page
  ...
  const handleNextPage = () => {
    if (validateForm() && isVerified) {
      if (paymentRequired) {
        ...
        // For non-payment required, directly call handleFinalSubmit
        handleFinalSubmit();
      }
    } else {
      showAlert(
          ...
          <div className={styles.formMain}>
            {!showPayment ? (
              <form>
                ...
                {isVerified && !showPayment && !paymentRequired && (
                  <div className={styles.formActions}>
                    <div className={styles.acknowledgement}>
                      <input
                        type="checkbox"
