...
const JoinNow = () => {
  ...
  const handleFinalSubmit = async () => {
    ...
    try {
      const response = await apiClient.post("final_submit/", {
        ...
        transaction_id: "No payment required", // No payment required
      })

      showAlert("success", "Membership registration successful, Membership activation upon approval")
      console.log(response, "Final submission response")
      setShowSuccessMessage(true)
    } catch (error) {
      console.error("Final submission failed:", error)
      showAlert("error", `Final submission failed: ${error.message || "Unknown error"}`)
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleSendOTP = async () => {
    ...
    try {
      ...
      showAlert("error", `Failed to send OTP: ${error?.response?.data?.error || "Unknown error"}`)
      setIsFormDisabled(false)
    } finally {
      setLoadingOtp(false)
    }
  ...
  const handleVerifyOTP = async () => {
    ...
    try {
      ...
      showAlert("error", `OTP verification failed: ${error?.response?.data?.error || "Unknown error"}`)
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleNextPage = () => {
    if (validateForm() && isVerified) {
      ...
      showAlert("warning", "Please fill all fields and verify your email before proceeding.")
    }
  }
    ...
    if (name === "uid") {
      ...
      if (trimmedValue.length >= 9) {
        ...
        if (!uidPattern.test(trimmedValue)) {
          ...
          showAlert("warning", "UID must start with 2 digits, followed by 3 letters, and end with 4 to 7 digits")
        } else {
  ...
  if (showSuccessMessage) {
    return (
      <div className={styles.successMessage}>
        ...
        <p>
          Membership registration successful, Membership activation upon approval. Thank you for joining CU-Intranet.
        </p>
          ...
          <div className={styles.formMain}>
            <form>
              <section className={styles.formSection}>
                <h3 className={styles.formHeaderTwo}>Join as a new member </h3>
                ...
                <div className={styles.inputGrid}>
                  ...
                  <div className={styles.inputGroup}>
                    ...
                    {mobileNumber && mobileNumber.length !== 10 && (
                      <span className={styles.error}>Mobile number must be 10 digits</span>
                    )}
                  </div>
                </div>
              </section>
              ...
              <section className={styles.formSection}>
                ...
                <div className={styles.inputGrid}>
                  <div className={styles.inputGroup}>
                    ...
                    {errors.entity && <span className={styles.error}>{errors.entity}</span>}
                  </div>
                    ...
                    <label>
                      ...
                      {isVerified && (
                        <span className={styles.verifiedBadge}>
                          <img src={verified || "/placeholder.svg"} alt="Verified" />
                        </span>
                      )}
                    </label>
                    <div className={styles.emailField}>
                      <input type="text" value={email} readOnly className={styles.formInput} disabled />
              ...
              {!isVerified && (
                <div className={styles.formActions}>
                  <button
                    ...
                    disabled={!name || !uid || !department || !mobileNumber || !entity || !entityType || !otp}
                  >
                ...
                <div className={styles.formActions}>
                  <div className={styles.acknowledgement}>
                    ...
                    <label htmlFor="acknowledge">
                      I acknowledge that all the information provided is correct and complete
                    </label>
                  </div>
                  <button
                    type="button"
