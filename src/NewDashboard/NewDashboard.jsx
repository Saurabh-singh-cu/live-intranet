...
//     // Get the month number (1-12) from the current month name
//     const monthNames = [
//       "January",
//       "February",
//       "March",
...
//     // Call getCalenderEvent with the current month
//     getCalenderEvent(monthNumber);
...
//   const getCalenderEvent = async (month = 1) => {
//     try {
//       const response = await apiClient.get(
//         `/api/events/monthly-approved-events/?month=${month}`
//       );
//       console.log(response, "Calender");

//       if (response.data && Array.isArray(response.data)) {
//         // Map the API response to our events format
//         const mappedEvents = response.data.map((event) => ({
//           id: event.id || Math.random().toString(36).substr(2, 9),
//           date: event.event_date || event.date,
//           title: event.title || event.event_name,
//           time: event.time || `${event.start_time} - ${event.end_time}`,
//           price: event.price || (event.is_paid ? `₹${event.price}` : "Free"),
//           ticketsLeft: event.tickets_left || event.available_seats || 50,
//           attendees: event.attendees || [],
//           color: event.color || getRandomColor(),
//         }));

//         // Update the events state with the mapped data
...
const NewDashboard = () => {
  ...
  const entityTypeColors = {
    CLUB: "#FF9800", // Orange
    "DEPARTMENT SOCIETY": "#4CAF50", // Green
    "PROFESSIONAL SOCIETY": "#2196F3", // Blue
  ...
  // Generate calendar days for current month with events from API
  const generateCalendarDays = () => {
    const year = currentYear;
    const month = new Date(`${currentMonth} 1, ${currentYear}`).getMonth();
    ...
    // Current month days
    for (let i = 1; i <= daysInMonth; i++) {
      const dateString = formatDateString(year, currentMonth, i);

      // Get events for this date from calendarEvents
      const dayEvents = calendarEvents[dateString] || [];

      // Map API events to our event format
      const formattedEvents = dayEvents.map((event) => ({
        id: `${event.entity_id}-${event.event_name}-${dateString}`,
        date: dateString,
        title: event.event_name,
        time: `${new Date(event.start_date).toLocaleDateString()} - ${new Date(
          event.end_date
        ).toLocaleDateString()}`,
        entity_type: event.entity_type_name,
        activity_type: event.activity_type,
        color:
          entityTypeColors[event.entity_type_name] || entityTypeColors.DEFAULT,
  ...
  // Replace the getCalenderEvent function with this implementation that uses your actual API response format
  const getCalenderEvent = async (month = 5) => {
    try {
      const response = await apiClient.get(
        `/api/events/monthly-approved-events/?month=${month}`
      );
      ...
      // Store the raw calendar events data as received from the API
      setCalendarEvents(response.data);

      // Process events for the events state (used elsewhere in the component)
      const allEvents = [];
      Object.keys(response.data).forEach((date) => {
        response.data[date].forEach((event) => {
          allEvents.push({
            id: `${event.entity_id}-${event.event_name}-${date}`,
            date: date,
            title: event.event_name,
            time: `${new Date(
              event.start_date
            ...
            attendees: [],
            color:
              entityTypeColors[event.entity_type_name] ||
                      ...
                      >
                        ...
                        <div className={styles.eventDetails}>
                          <div className={styles.eventType}>
                            ...
                            <span className={styles.eventActivityType}>
                              {event.activity_type}
                            </span>
                          </div>
                        </div>
                          ...
                          <div className={styles.eventExpandedDetails}>
                            <div className={styles.eventDates}>
                              <div className={styles.eventDateItem}>
                                <span className={styles.eventDateLabel}>
                                  Start:
                                </span>
                                <span className={styles.eventDateValue}>
                                  {new Date(
                                    event.start_date
                                  ).toLocaleDateString()}
                                </span>
